  ...
  for (let i = 0; i < array.length; i++) {
    ...
    await fetch(window.Shopify.routes.root + `products/${element}.js`)
      ...
      .then((product) => {
        ...
        const discountTag = Math.floor(
          (((price / 100) * 100) / (compare_at_price_min / 100) - 100) * -1
        );
        ...
        item.classList.add(
          ...
          "relative",
          "transition-shadow",
          "hover:shadow-[0_6px_20px_0px_#0000000F]"
        );
        item.innerHTML = `
  <div class="wrapper-shelf relative">
    ...
    <div class="pdp-img relative flex items-center justify-center px-4">
      <a class="image-wrapper [&>img]:max-w-[205px] [&>img]:max-h-[205px]" href="${url}">
         <img src="${image[0].src}" width="427" height="650" alt="${title}" class="rounded-tl-lg rounded-tr-lg w-full max-w-48 max-h-48 mx-auto">
      </a>
